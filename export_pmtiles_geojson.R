...
OUTPUT_GEOJSON <- "paris_buildings_aci_v3.geojson"
...
# --- 2b. Compute year-over-year change contributors ---
# Pivot the absolute counts and contributions
wide_contribs <- joined_data %>%
  select(id, Year, CODE_ACTIVITE, count, contribution) %>%
  pivot_wider(
    id_cols = c(id, CODE_ACTIVITE),
    names_from = Year,
    values_from = c(count, contribution),
    values_fill = 0
  )

change_contributors <- data.frame(id = character())
...
for (i in 1:(length(years)-1)) {
  for (j in (i+1):length(years)) {
    ...
    col_suffix <- paste0("_", substr(base, 3, 4), substr(end, 3, 4))
    contrib_base_col <- paste0("contribution_", base)
    contrib_end_col <- paste0("contribution_", end)
    count_base_col <- paste0("count_", base)
    count_end_col <- paste0("count_", end)
    
    # Calculate difference for this pair
    pair_changes <- wide_contribs %>%
      mutate(
        change_val = .data[[contrib_end_col]] - .data[[contrib_base_col]],
        count_diff = .data[[count_end_col]] - .data[[count_base_col]]
      ) %>%
      group_by(id) %>%
      arrange(desc(change_val)) %>%
      mutate(
        rank_pos = row_number(),
        rank_neg = rev(rank_pos)
      ) %>%
      filter(rank_pos <= 3 | rank_neg <= 3) %>%
      mutate(label = case_when(
        rank_pos == 1 ~ paste0("chg_first", col_suffix),
        rank_pos == 2 ~ paste0("chg_second", col_suffix),
        rank_pos == 3 ~ paste0("chg_third", col_suffix),
        rank_neg == 1 ~ paste0("chg_last", col_suffix),
        rank_neg == 2 ~ paste0("chg_seclast", col_suffix),
        rank_neg == 3 ~ paste0("chg_thirdlast", col_suffix)
      )) %>%
      ungroup() %>%
      pivot_wider(
        id_cols = id,
        names_from = label,
        values_from = c(CODE_ACTIVITE, change_val, count_diff),
        names_glue = "{.value}_{label}"
      )
    
    if (nrow(change_contributors) == 0) {
      change_contributors <- pair_changes
    } else {
      change_contributors <- full_join(change_contributors, pair_changes, by = "id")
    }
  }
}

final_contributors2 <- left_join(final_contributors2, change_contributors, by = "id")
...
# tippecanoe -o /mnt/e/ACI_app/ACI_Paris/paris_buildings_aci_v3.pmtiles   --project=EPSG:4326  --drop-densest-as-needed -Z 7 -zg --smallest-maximum-zoom-guess=14 -kg -rp -l points_layer /mnt/e/ACI_app/ACI_Paris/paris_buildings_aci_v3.geojson
